# Multiplex PCR Primer Designer (v5.0 "Smart Pools")
...
It is an end-to-end solution that takes a genome, a list of gene targets, and produces a final, validated, and sequencing-ready primer list. **This version uses an advanced 3'-end stability check to design highly efficient, large-scale pools, avoiding false-positive clashes from older methods.** The script automatically detects whether your design is "sparse" or "tiled" (overlapping) and applies the optimal design strategy to ensure the highest probability of success at the lab bench.
...
* **Intelligent 3'-End Validation:** Replaces older, "dumb" dimer/hairpin checks. All primers are validated for:

  1. **Genome Specificity** (via local BLAST)

  2. **True 3'-End Stability** (via `primer3.calc_end_stability`)

* This "smart" validation **only flags biochemically problematic 3'-extendable interactions** (hairpins or dimers) and ignores harmless 5' interactions, dramatically reducing false positives and enabling larger, more efficient pools.

* **Smart Retry Logic:** Automatically retries failed targets with different design parameters (size, $T_m$, etc.).

* **Intelligent Auto-Pooling (v5.0):** For sparse or tiled designs, this is the default mode. It finds the *minimum number of compatible pools* (N-pools) required to achieve a **perfect, 0-clash** design (based on the new 3'-end logic), minimizing your lab burden and often resulting in a single pool.
...
graph TD
    ...
    F -- False, Default --> E(Run Auto-N-Pool Logic v5.0)
...
* This file contains a complete record of the design process. It lists all targets that failed the initial design and, most importantly, provides the full list of all potential 3' dimer clashes that the "Auto-Healing" logic had to resolve (if any).
